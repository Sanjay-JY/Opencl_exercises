...
int main() {
    ...
    std::cout<<"1 for clockwise rotation and 2 for anticlockwise rotation: ";
    std::cin>>choice;
    if(choice==1)
    {
        std::ifstream sourceFile("clockwise.cl");
        ...
        // Create output image
        cv::Mat outputMat(width, height, CV_8UC1, outputImage.data());

        // Display output image
        cv::imshow("Output Image", outputMat);
        cv::imwrite("clockwise.jpg", outputMat);
    ...
    {
        std::ifstream sourceFile("anticlockwise.cl");
        if (!sourceFile.is_open()) {
            std::cerr << "Failed to open OpenCL source file." << std::endl;
            return {};
        }
        ...
        // Display output image
        cv::imshow("Output Image", outputMat);
        cv::imwrite("Anticlockwise.jpg", outputMat);
